...
class ActorCriticModel(nn.Module):
    ...
    def forward(self, obs: torch.tensor, recurrent_cell: torch.tensor, device: torch.device, sequence_length: int = 1):
        ...
        if sequence_length == 1:
            ...
            h, recurrent_cell = self.recurrent_layer(
                h.unsqueeze(1), recurrent_cell)
            ...
            h = h.reshape((h_shape[0] // sequence_length),
                          sequence_length, h_shape[1])
    ...
    def init_recurrent_cell_states(self, num_sequences: int, device: torch.device) -> tuple:
        ...
        """
        hxs = torch.zeros(
            (num_sequences), self.recurrence["hidden_state_size"], dtype=torch.float32, device=device).unsqueeze(0)
        cxs = None
